...
export default function ClientSidebar({ isOpen, onClose, user }) {
  ...
  const tableItems = [
    { path: '/client/daily-work', icon: Calendar, label: 'Daily Work' },
    { path: '/client/view-tables', icon: Eye, label: 'View Tables' },
  ]

  const quickAccessItems = [
    { path: '/client/supplier-info', icon: Package, label: 'Supplier Info' },
    { path: '/client/material-inquiry', icon: HelpCircle, label: 'Material Inquiry' },
    { path: '/client/customer-delivery', icon: Truck, label: 'Customer Delivery' }
  ]
  ...
  const handleLinkClick = (e, path) => {
    e.preventDefault()
    console.log('Client navigating to:', path)
  ...
  return (
    <>
      ...
      <div className={`fixed inset-y-0 left-0 z-50 w-64 bg-white shadow-xl transform ${
        ...
        {/* Logo Section */}
        <div className="flex items-center justify-between h-16 px-4 border-b bg-gradient-to-r from-green-600 to-green-700">
          <div className="flex items-center space-x-2">
            ...
            <div className="flex flex-col">
              <span className="text-lg font-semibold text-white leading-tight">
                {user?.companyName?.split(' ')[0] || 'Client'}
              </span>
        ...
        <nav className="flex-1 px-4 py-4 space-y-1 overflow-y-auto scrollbar-hide">
            ...
            {menuItems.map((item) => (
              ...
              >
                <item.icon className={`w-5 h-5 mr-3 transition-colors ${
                  isActive(item.path) ? 'text-green-700' : 'text-gray-500 group-hover:text-gray-700'
                }`} />
                {item.label}
              </button>
            ))}
          ...
          <div className="pt-6">
            <div className="px-3 py-4 bg-green-50 rounded-lg">
              <h4 className="text-sm font-semibold text-green-800 mb-2">Company Info</h4>
              <div className="text-xs text-green-600 space-y-1">
                <p><strong>Company:</strong> {user?.companyName}</p>
                <p><strong>Client ID:</strong> {user?.clientId}</p>
                <p><strong>Status:</strong> <span className="text-green-700 font-medium">Active</span></p>
              </div>
            </div>
          </div>
        </nav>
